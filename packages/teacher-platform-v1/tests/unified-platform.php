...
$newClass=SchoolDirectory::createClass($both,1,'QA Einstieg','2026/27');
$roomStates=[];$createdRooms=0;
$factory=function(string $label,int $expires) use (&$roomStates,&$createdRooms,$both): array {
    $codes=['STU234','STU235','STU236','STU237','STU238','STU239','STU242','STU243'];
    $state=['code'=>$codes[$createdRooms++],'label'=>$label,'ownerUserId'=>(int)$both['id'],'createdAt'=>time(),'expiresAt'=>$expires,'materialAccessKey'=>str_repeat('b',48)];
    $roomStates[$state['code']]=$state;return $state;
};
$reader=function(string $code) use (&$roomStates): array {return $roomStates[$code];};
$startInput=['mode'=>'class','class_id'=>$newClass['id'],'request_id'=>str_repeat('a',32),'label'=>'Ethik QA'];
$start=ReligionPlatform\TeachingStart::start($both,$slug,$startInput,$factory,$reader);
$startedAssignment=$start['teaching']['assignment_id'];
check($start['state']['code']==='STU234' && $startedAssignment!==null,'class start creates a connected assignment and live room in one operation');
$retention=ReligionPlatform\LearningWork::assertAssignment($workTeacher,$startedAssignment)['retain_until'];
$again=ReligionPlatform\TeachingStart::start($both,$slug,array_replace($startInput,['request_id'=>str_repeat('c',32),'retention_days'=>730]),$factory,$reader);
check($createdRooms===1 && $again['teaching']['assignment_id']===$startedAssignment && $again['teaching']['resumed'],'class resume and lost-response retry reuse the same assignment and room');
check(ReligionPlatform\LearningWork::assertAssignment($workTeacher,$startedAssignment)['retain_until']===$retention,'resume never silently extends personal retention');
check(!str_contains($start['state']['label'],'QA Einstieg'),'public room label does not disclose the class name');
rejects(fn()=>ReligionPlatform\TeachingStart::start($foreign,$slug,$startInput,$factory,$reader),'foreign teacher cannot start another organisation class');
SchoolDirectory::setTeacher($admin,$newClass['id'],(int)$assessment['id'],'viewer');
Identity::setProducts($admin,$e['subject'],['learning','assessment']);
rejects(fn()=>ReligionPlatform\TeachingStart::start($assessment,$slug,$startInput,$factory,$reader),'viewer cannot start live teaching for the class');
rejects(fn()=>ReligionPlatform\TeachingStart::start($both,$slug,array_replace($startInput,['mode'=>'']),$factory,$reader),'missing mode never silently starts an anonymous room');
rejects(fn()=>ReligionPlatform\TeachingStart::start($both,$slug,array_replace($startInput,['mode'=>'temporary']),$factory,$reader),'anonymous start rejects an attached personal class');
$temporary=['mode'=>'temporary','request_id'=>str_repeat('d',32)];
$anon=ReligionPlatform\TeachingStart::start($both,$slug,$temporary,$factory,$reader);
$anonAgain=ReligionPlatform\TeachingStart::start($both,$slug,$temporary,$factory,$reader);
check($anon['teaching']['assignment_id']===null && $anonAgain['state']['code']===$anon['state']['code'] && $createdRooms===2,'temporary start is repeatable and creates no personal assignment');
check($anon['state']['expiresAt']-time()<=8*3600,'temporary teaching has a short default lifetime');
ReligionPlatform\Rooms::end('STU234',$both);
$continued=ReligionPlatform\TeachingStart::start($both,$slug,$startInput,$factory,$reader);
check($continued['teaching']['assignment_id']===$startedAssignment && $continued['state']['code']!=='STU234','ended live room is replaced while its personal assignment remains the same');
$extra=ReligionPlatform\LearningWork::createAssignment($both,['class_id'=>$newClass['id'],'module_slug'=>$slug,'label'=>'Weiterer Verlauf']);
rejects(fn()=>ReligionPlatform\TeachingStart::start($both,$slug,$startInput,$factory,$reader),'ambiguous existing assignments require explicit selection');
$explicit=ReligionPlatform\TeachingStart::start($both,$slug,array_replace($startInput,['assignment_id'=>$startedAssignment]),$factory,$reader);
check($explicit['teaching']['assignment_id']===$startedAssignment,'explicit assignment selection resumes exactly the requested work');
rejects(fn()=>ReligionPlatform\Rooms::mirror('STU234',$slug,$foreign,['expiresAt'=>time()+3600]),'global room-code collision cannot change another teacher room');
SchoolDirectory::setTeacher($admin,$newClass['id'],(int)$assessment['id'],'editor');
$otherFactory=function(string $label,int $expires) use ($factory,&$roomStates,$assessment): array {$state=$factory($label,$expires);$state['ownerUserId']=(int)$assessment['id'];$roomStates[$state['code']]=$state;return $state;};
$parallel=ReligionPlatform\TeachingStart::start($assessment,$slug,$startInput,$otherFactory,$reader);
check($parallel['state']['code']!==$explicit['state']['code'] && $parallel['teaching']['assignment_id']!==$startedAssignment,'two authorised teachers in the same class have independent live teaching and work assignments');
$adminClass=SchoolDirectory::createClass($admin,2,'QA andere Schule','2026/27');
$adminFactory=function(string $label,int $expires) use ($factory,&$roomStates,$admin): array {$state=$factory($label,$expires);$state['ownerUserId']=(int)$admin['id'];$roomStates[$state['code']]=$state;return $state;};
$crossSchool=ReligionPlatform\TeachingStart::start($admin,$slug,array_replace($startInput,['class_id'=>$adminClass['id']]),$adminFactory,$reader);
check((int)ReligionPlatform\Rooms::find($crossSchool['state']['code'])['organisation_id']===2,'administrator start uses the selected class organisation rather than the account default');
$expiredJournal=$temp.'/teaching-starts/'.str_repeat('e',64).'.json';
$lockedJournal=$temp.'/teaching-starts/'.str_repeat('f',64).'.json';
file_put_contents($expiredJournal,json_encode(['expires_at'=>time()-2*86400]));file_put_contents($lockedJournal,json_encode(['expires_at'=>time()-2*86400]));
$journalLock=fopen($lockedJournal,'r+');flock($journalLock,LOCK_EX);
$maintenance=ReligionPlatform\Maintenance::run();
check($maintenance['teaching_start_files']===1 && !file_exists($expiredJournal),'maintenance removes expired start metadata after its safety window');
check(file_exists($lockedJournal),'maintenance leaves an in-flight teaching start untouched');
flock($journalLock,LOCK_UN);fclose($journalLock);
