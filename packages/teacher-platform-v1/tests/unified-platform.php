...
$teacherImport=$input;$teacherImport['class']['external_id']='103';$teacherImport['class']['label']='12D';$teacherImport['class']['learners'][0]['external_id']='203';$teacherImport['class']['learners'][0]['username']='12D01';
$teacherResult=ReligionPlatform\DirectoryBridge::import(['identity'=>$e,'client'=>$client],$teacherImport);
check((bool)$teacherResult['class_id'],'assessment teacher can publish a new owned app class without administrator assistance');
...
check((int)$db->query('SELECT COUNT(*) FROM platform_learners')->fetchColumn()===3,'repeated import never duplicates learners');
...
check((int)$db->query('SELECT COUNT(*) FROM platform_classes')->fetchColumn()===3,'failed import is atomic');
...
$parsedRoster=ReligionPlatform\RosterImport::parseText("Listennummer;Nachname;Vorname\n2;Student;Second\n");
check($parsedRoster===[['number'=>2,'name'=>'Second Student']],'clipboard parser recognises headers, numbers and name columns');
Identity::setProducts($admin,$b['subject'],['learning','assessment']);
$rosterPlan=SchoolDirectory::previewRoster($both,$class['id'],$parsedRoster,['learning','assessment']);
check(!$rosterPlan['conflicts'] && $rosterPlan['learners'][0]['username']==='13RELIGION02','roster preview creates deterministic usernames without writing');
$rosterImport=SchoolDirectory::importRoster($both,$class['id'],$parsedRoster,['learning','assessment']);
check(count($rosterImport['credentials'])===1 && count(SchoolDirectory::learners($both,$class['id']))===2,'confirmed roster import creates the student once');
$sameRoster=SchoolDirectory::importRoster($both,$class['id'],$parsedRoster,['learning','assessment']);
check(!$sameRoster['credentials'],'repeated roster import keeps the existing stable identity');
...
$startLockPath=$temp.'/teaching-starts/'.hash('sha256',implode('|',[(string)$both['id'],$slug,'class',$newClass['id']])).'.json';
$startLock=fopen($startLockPath,'r+');flock($startLock,LOCK_EX);
$beforeBusy=$createdRooms;
rejects(fn()=>ReligionPlatform\TeachingStart::start($both,$slug,array_replace($startInput,['assignment_id'=>$startedAssignment]),$factory,$reader),'concurrent start is rejected while the same teacher/class lock is held');
check($createdRooms===$beforeBusy,'rejected concurrent start does not create an extra live room');
flock($startLock,LOCK_UN);fclose($startLock);
$retry=ReligionPlatform\TeachingStart::start($both,$slug,array_replace($startInput,['assignment_id'=>$startedAssignment]),$factory,$reader);
check($retry['state']['code']===$explicit['state']['code'],'retry after concurrent start resumes the original room');
$failureClass=SchoolDirectory::createClass($both,1,'QA Fehlerpfad','2026/27');
$failureInput=array_replace($startInput,['class_id'=>$failureClass['id']]);
$faultyFactory=function(string $label,int $expires) use ($factory): array {$factory($label,$expires);throw new RuntimeException('Synthetic interruption after physical create');};
$assignmentsBefore=(int)$db->query('SELECT COUNT(*) FROM learning_assignments')->fetchColumn();
rejects(fn()=>ReligionPlatform\TeachingStart::start($both,$slug,$failureInput,$faultyFactory,$reader),'interrupted physical room creation is reported and never switches to anonymous mode');
$recovered=ReligionPlatform\TeachingStart::start($both,$slug,$failureInput,$factory,$reader);
check((int)$db->query('SELECT COUNT(*) FROM learning_assignments')->fetchColumn()===$assignmentsBefore+1 && $recovered['teaching']['mode']==='class','retry after interrupted room creation retains one personal assignment');
check(ReligionPlatform\LearningWork::read(Identity::find($student['subject']),$assignment)['revision']===2,'failure recovery never removes existing personal work');
